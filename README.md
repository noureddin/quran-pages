...
- `2/getpages.sh`: a Bash script using ImageMagick and Wget to download the page images from IslamicBook,
...
- `2/pages/776x1053-webp/`: the compressed page images created with `2/convert.sh`.
  (The original JPGs are not included; use `2/getpages.sh` if you need them.)

  Note: File names are NOT zero-padded; eg, the first page is at `2/pages/776x1053-webp/1.webp`.

## Included primary data

- `2/words.json`: a JSON file whose root element is a 604-element array representing the pages,
  ...
  It's also available compressed in `2/words.json.zst`.
  (I personally use [fzstd](https://github.com/101arrowz/fzstd) on the Web.)

- `2/lastwords.json`: a JSON file whose root element is a 604-element array representing the pages,
  each element is an array of "lines" in the page,
  each "line" is the index of the word that ends that line.

  For example, `lastwords[0]` is `[0, 2, 8, ...]`, that means
  ...
  It's also available compressed in `2/lastwords.json.zst`.

- `2/ayat.json`: a JSON file whose root element is a 604-element array representing the pages,
  ...
  It's also available compressed in `2/ayat.json.zst`.

- `2/pauses.json`: a JSON file whose root element is a 604-element array representing the pages,
  ...
  It's also available compressed in `2/pauses.json.zst`.

  Note: Split waqf (الوقف المتعانق) is recorded only for the one with the gap in the Dar-ul-Ma‘refa Muṣħaf;
  the other one is moved to `2/lessersplits.json` described below.

- `2/morepauses.json`: a (work-in-progres) JSON file in the same format as `pauses.json`.
  Please read [morepauses.description](2/morepauses.description) for info and details.

  It's also available compressed in `2/morepauses.json.zst`.

## Included metadata and related scripts

These all are tiny files. You can embed the files you need, or fetch the combined
`2/allmeta.json` descripted below (or its compressed version `2/allmeta.json.zst`).

- `2/numberofwords.json`: a JSON file whose root element is a 604-element array representing the pages,
  each element contains just the number of elements of the corresponding array in `2/words.json`.

- `2/numberofayat.json`: a JSON file whose root element is a 604-element array representing the pages,
  each element contains just the number of elements of the corresponding array in `2/ayat.json`.

- `2/margins.json`: a JSON file whose root element is an object
  ...
  The values are tuples exactly like `2/words.json`.

- `2/marginwords.json`: a JSON file whose root element is an object
  ...
  whose `idx` key has the index of the corresponding word in `2/words.json` in the page,
  and whose `dim` key has a dimension tuple exactly like `2/words.json`.
...
- `2/colors.json`: a JSON file containing the background colors of the words and the margins,
...
- `2/lessersplits.json`: a JSON file whose root element is an object
  ...
  the favoured ones have a gap after them and are included in primary `2/pauses.json` instead.

- `2/suarstarts.json`: a JSON file whose root element is a 114-element array representing the suar,
...
- `2/headers.json`: a JSON file whose root element is a 604-element array representing the pages,
  each element is an array containing the word index of any header (sura name) in that page.

- `2/basmalaat.json` (note the two `a`s): a JSON file whose root element is a 604-element array representing the pages,
...
- `2/allmeta.json`: includes all other metadata JSONs (ie, not the primary data) as values in the root object;
  eg., `"marginwords"` is a key whose value is the contents of the JSON file with the same name.

  It's also available compressed in `2/allmeta.json.zst`.

  For convenience, `2/allmeta.json` also includes two additional key-value pairs:
  the dimensions of the pages in pixels: `"width": 776` and `"height": 1053`.

- `2/make.sh`: generates `allmeta.json`, and compresses it and the primary data JSON files.

- `2/generate.pl`: generates half of the metadata JSON files from the primary data and the other half.

- `2/jq.sh`: contains generating commands for some other probably useful formats of the same data.
